...
(function () {
  ...
  // How many questions to draw (at random) from a week's quiz question pool
  // each time the quiz is opened, so retaking it after re-studying doesn't
  // just show the exact same test again.
  const QUIZ_SAMPLE_SIZE = 10;

  // Fisher-Yates shuffle that returns a NEW array (never mutates the input --
  // important since the input is often the original curriculum data).
  ...
  // Builds one quiz attempt from a week's full question pool: picks a random
  // subset (or all of them, if the pool is smaller than the sample size),
  // shuffles their order, AND shuffles each question's choice order (so the
  // correct answer isn't always in the same position either) -- while
  // leaving the original curriculum data completely untouched.
  function sampleQuizQuestions(pool, sampleSize) {
    const picked = shuffledCopy(pool).slice(0, Math.min(sampleSize, pool.length));
    return picked.map((q) => {
      const order = shuffledCopy(q.choices.map((_, i) => i));
      return {
        question: q.question,
  ...
  // Wraps the first occurrence of `target` inside `text` with a highlight span
  // (case-insensitive match). Falls back to plain escaped text if not found.
  function highlightOccurrence(text, target, className) {
    const safeText = String(text == null ? "" : text);
    const safeTarget = String(target == null ? "" : target).trim();
    if (!safeTarget) return escapeHtml(safeText);
    const idx = safeText.toLowerCase().indexOf(safeTarget.toLowerCase());
    if (idx === -1) return escapeHtml(safeText);
    const before = safeText.slice(0, idx);
    const match = safeText.slice(idx, idx + safeTarget.length);
    const after = safeText.slice(idx + safeTarget.length);
    return `${escapeHtml(before)}<span class="${className}">${escapeHtml(match)}</span>${escapeHtml(after)}`;
  }
  ...
  function openLesson(trackId, week) {
    ...
    weekData.grammar.examples.forEach((ex) => {
      // Support both the legacy plain-string format and the new
      // { en, th, highlight } object format.
      const exObj = typeof ex === "string" ? { en: ex, th: "", highlight: "" } : ex;
      const li = document.createElement("li");
      li.innerHTML = `
        <div class="example-en-row">
          <span class="example-en">${highlightOccurrence(exObj.en, exObj.highlight, "hl-grammar")}</span>
          <button type="button" class="speak-btn" aria-label="ฟังเสียง">🔊</button>
        </div>
        ${exObj.th ? `<div class="example-th muted">${escapeHtml(exObj.th)}</div>` : ""}
      `;
      li.querySelector(".speak-btn").addEventListener("click", () => speak(exObj.en));
    ...
    weekData.vocabulary.forEach((v) => {
      ...
      card.innerHTML = `
        ...
        <div class="vocab-example">${highlightOccurrence(v.example, v.en, "hl-vocab")}</div>
        ${v.exampleTh ? `<div class="vocab-example-th">${escapeHtml(v.exampleTh)}</div>` : ""}
      `;
    ...
    if (weekData.story && Array.isArray(weekData.story.sentences) && weekData.story.sentences.length > 0) {
      storyBlock.hidden = false;
      $("#storyTitle").textContent = weekData.story.title || "";
      const storyContainer = $("#storyContainer");
      storyContainer.innerHTML = "";
      weekData.story.sentences.forEach((s) => {
        const row = document.createElement("div");
        row.className = "story-sentence";
        row.innerHTML = `
          <div class="story-en-row">
            <span class="story-en">${escapeHtml(s.en)}</span>
            <button type="button" class="speak-btn" aria-label="ฟังเสียง">🔊</button>
          </div>
          <div class="story-th">${escapeHtml(s.th)}</div>
        `;
        row.querySelector(".speak-btn").addEventListener("click", () => speak(s.en));
        storyContainer.appendChild(row);
      });
    } else {
  ...
  let speakingPhraseIdx = 0;

  // Supports both the new { prompt, phrases: [...] } shape and the legacy
  // single-phrase { prompt, targetPhrase } shape, so a week that hasn't been
  // migrated to the multi-phrase format yet still works.
  function getSpeakingPhrases(weekData) {
    if (weekData.speaking.phrases && weekData.speaking.phrases.length) return weekData.speaking.phrases;
    if (weekData.speaking.targetPhrase) return [weekData.speaking.targetPhrase];
    return [];
  }
  ...
  function getCurrentSpeakingPhrase() {
    ...
    if (!weekData) return "";
    const phrases = getSpeakingPhrases(weekData);
    return phrases[speakingPhraseIdx] || "";
  }
  ...
  function renderSpeakingPhrase() {
    ...
    if (speakingPhraseIdx >= total) speakingPhraseIdx = total - 1;
    if (speakingPhraseIdx < 0) speakingPhraseIdx = 0;

    $("#targetPhrase").textContent = phrases[speakingPhraseIdx] || "";
    $("#speakingProgressLabel").textContent = total > 1 ? `ประโยคที่ ${speakingPhraseIdx + 1} / ${total}` : "";
    $("#speakingFeedback").textContent = "";

    const prevBtn = $("#btnPrevPhrase");
    const nextBtn = $("#btnNextPhrase");
    if (prevBtn) prevBtn.disabled = speakingPhraseIdx === 0;
    if (nextBtn) nextBtn.disabled = speakingPhraseIdx >= total - 1;

    // Switching phrases mid-recording would otherwise leave a recognition
    // session running against whatever phrase the user has since moved away
    // from -- stop it so Play/Record always act on the newly shown phrase.
    if (activeSpeechRecognition) activeSpeechRecognition.stop();
  }

  function setupSpeakingButtons() {
    $("#btnPlayPhrase").addEventListener("click", () => {
      const phrase = getCurrentSpeakingPhrase();
    ...
    $("#btnRecordPhrase").addEventListener("click", () => {
      const weekData = getCurrentLessonWeekData();
      if (!weekData) return;
      const targetPhrase = getCurrentSpeakingPhrase();
      if (!targetPhrase) return;
      const feedback = $("#speakingFeedback");
      const btn = $("#btnRecordPhrase");

      // Second click while listening = manual stop.
      ...
      let finalTranscript = "";
      feedback.textContent = "🎙️ กำลังฟัง... พูดประโยคด้านบนได้เลย (กดปุ่มอีกครั้งเมื่อพูดจบ)";
      btn.textContent = "⏹️ หยุดพูด";
      ...
      rec.onresult = (e) => {
        ...
        feedback.textContent = `🎙️ ${finalTranscript}${interim ? " " + interim : ""}`;
      };
      ...
      rec.onend = () => {
        btn.textContent = "🎤 พูดตาม";
        btn.classList.remove("recording");
        activeSpeechRecognition = null;

        const transcript = finalTranscript.trim();
        if (!transcript) {
          feedback.textContent = "ไม่ได้ยินเสียงพูด ลองพูดใหม่อีกครั้ง";
          return;
        }
        const score = similarityScore(targetPhrase, transcript);
  ...
  function openQuiz(trackId, week) {
    ...
    const weekData = getCurrentLessonWeekData();
    if (!weekData) return;
    quizQuestions = sampleQuizQuestions(weekData.quiz, QUIZ_SAMPLE_SIZE);
  ...
  // Replaces the emoji characters inside `container` with real Twemoji image
  // files pulled from a CDN, so every word gets the same colorful picture on
  // every OS/browser instead of depending on the system's own emoji font
  // (which can render blank/monochrome/missing on some platforms).
  // Silently does nothing if the Twemoji script failed to load (e.g. no
  // network) -- the plain emoji character still shows as a safe fallback.
  function renderVocabEmojiAsImages(container) {
    ...
    try {
      twemoji.parse(container, {
        ...
        className: "emoji vocab5000-emoji-img",
        onerror: function () {
          // CDN hiccup for this one icon -- fall back to the original emoji
          // character instead of showing a broken-image icon.
          this.onerror = null;
